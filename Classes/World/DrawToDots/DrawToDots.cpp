...
#define ITBRUSH std::vector<Brush*>::iterator 

void freeBrushs(std::vector<Brush*>& brushs, bool forceAll = false) {
	bool begin = false;
	for (ITBRUSH it = brushs.begin(), it_e = brushs.end(); it != it_e; ++it) {
		if (forceAll) {
			delete (*it);
			continue;
		}
...
void endBrush(Brush* brush, int i = -1) {
	if (brush->m_type == Brush::BrushType::NO) {
		brush->toEnd(.01f, _delayFromEnd);
	}
	else if (brush->m_type == Brush::BrushType::YES) {
		//todo
		//плавный уход
		//или линейный, сразу всё
		if (i >= 0)
			brush->toEnd(i*.002f, _delayFromEnd);
		else
			brush->toEnd(.02f, _delayFromEnd);
	}
	else if (brush->m_type == Brush::BrushType::HELPER) {
		brush->setType(Brush::BrushType::HIDE);
		brush->reset();
		brush->toEnd(i*.001f, _delayFromEnd);
	}
}

void endBrush(std::vector<Brush*>& brushs) {
	int i = 0;
